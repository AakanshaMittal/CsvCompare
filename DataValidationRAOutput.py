...
# ---------- SMART NUMERIC HANDLING (fixes E-notation issue) ----------

def normalize_code_like(x):
    if x is None:
        return ""
    s = str(x).strip()
 
    # If value is only digits → treat as code/number and remove leading zeros
...
def values_equal(v1, v2):
    n1 = normalize_code_like(v1)
    n2 = normalize_code_like(v2)
 
    d1, d2 = smart_decimal(n1), smart_decimal(n2)
...
def main():
    ...
    file1 = input("Enter the UI Exported File Relative Path : ")

    file2 = input("Enter the RPT File Relative Path : ")

    h1 = int(input("Enter the starting header for file 1 as 0-Based : "))

    h2 = int(input("Enter the starting header for file 2 as 0-Based : "))

    report_name = input("Enter the report output file name (eg: RA_Output.html): ")
    ...
    generate_html_report(extra1, extra2, mismatches, summary, ndc_map,
                         ...
                         Path(file1).name, Path(file2).name, report_name)
 
    print("Done. New report generated:", report_name)
